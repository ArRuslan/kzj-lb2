...
@WebServlet("/users/remove-group")
public class RemoveUserFromGroupServlet extends HttpServlet {
    private static final Log log = LogFactory.getLog(RemoveUserFromGroupServlet.class);
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException, ServletException {
        ...
        try {
            DatabaseManager.getInstance().removeUserFromGroup(userAndGroup.first, userAndGroup.second);
        } catch (SQLException exc) {
            log.error("Failed to remove user from group!", exc);
            req.setAttribute("error", "Failed to remove user from group");
